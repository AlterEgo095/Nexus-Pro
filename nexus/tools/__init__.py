...
class ToolOrchestrator:
    ...
    def __init__(self, max_parallel: int = 5, timeout: int = 120, retry_attempts: int = 1):
    ...
    async def execute_chain(
        self, tool_chain: List[Dict[str, Any]], query: str, media_results: List[Any]
    ) -> List[ToolResult]:
        """
    ...
    async def _execute_single(
        self, tool_spec: Dict[str, Any], previous_results: Optional[List[ToolResult]] = None
    ) -> ToolResult:
        """
        ...
        if self._is_circuit_open(tool_name):
            return ToolResult(
                tool_name=tool_name, success=False, result=None, error="Circuit breaker open"
            )
        ...
        for attempt in range(self.retry_attempts + 1):
            try:
                ...
                if attempt == self.retry_attempts:
                    ...
                    # Attempt fallback
                    fallback_result = await self._attempt_fallback(tool_spec, e, previous_results)
    ...
    async def _call_tool(
        self, tool_spec: Dict[str, Any], previous_results: Optional[List[ToolResult]]
    ) -> Any:
        """
