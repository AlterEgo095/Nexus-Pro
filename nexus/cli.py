...
Examples:
  ...
  nexus --config custom.yaml query "Analyze this"
    ...
    parser.add_argument("--version", action="version", version="NEXUS Ultimate 1.0.0")

    parser.add_argument("--config", type=str, help="Path to configuration file", default=None)

    parser.add_argument(
        "--mode", choices=["auto", "supervised", "debug"], default="auto", help="Execution mode"
    )
    ...
    query_parser.add_argument("--media", nargs="+", help="Media URLs to process")
    ...
    # Initialize agent
    agent = NexusAgent(config_path=args.config, mode=args.mode, platform=args.platform)
...
async def handle_query(agent, args):
    ...
    response = await agent.process(query=args.text, media_urls=args.media or [])
...
def run_tests():
    ...
    result = subprocess.run(
        ["pytest", "tests/", "-v", "--cov=nexus"], cwd=Path(__file__).parent.parent
    )
